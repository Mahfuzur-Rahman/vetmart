...
Executed inline on 2026-08-20 on branch `fix/server-side-persistence`. The plan
above was written before implementation; this section records where reality
diverged from it and what was added once the code was actually read.

### Deviations from the plan

**Task 1 — the guard keys on `VERCEL`, not `NODE_ENV`.** The plan said to reject
a localhost `DATABASE_URL` in production. That would have broken the BDIX VPS
target, where Postgres on the same box over localhost/socket is the *correct*
configuration per §4.1. The guard now fires only when `VERCEL=1`, where there is
definitively no local Postgres. Two further serverless guards were added at the
same time: `DB_POOL_MAX > 1` and `STORAGE_DRIVER=local`.

**Task 3 — `updateProduct` cannot reuse `productCreateSchema` wholesale.** The
batch/expiry requirement is a create-time invariant; a partial update that never
mentions a batch must not be rejected for it. `productUpdateSchema` therefore
derives from the inner object type and only assigns keys the caller actually
sent, so a partial update cannot blank fields it never mentioned.

**Task 4 — RBAC could not be applied as planned.** The plan assumed the admin
routes could simply be guarded. In fact `setAdminSession()` was never called
anywhere: the admin "logged in" entirely in the browser, and the session cookie
was scoped to path `/admin` so it would never have reached `/api/v1/admin/*`.
Adding a guard would have locked the owner out of their own panel. Real admin
authentication had to be built first (see below), so guarding was deferred to
its own commit rather than shipped half-working.

**Task 5 — the N+1 fix mattered more than expected.** Two queries per row with
`DB_POOL_MAX=1` serialize on serverless: a 48-item page issued ~96 sequential
round trips to Mumbai, against Vercel's 10s function limit.

### Work added beyond the plan

Discovered by reading the code rather than from the original diagnosis:

1. **Real admin authentication.** `POST /api/v1/admin/auth/login` verifying
   against the `admins` table, a logout route, the cookie path fix, and
   `lib/api/guard.ts`. The previous login accepted *any* password and fell back
   to the first super-admin account on an unknown email. The RBAC redirect in
   the admin layout was also commented out, leaving the whole panel public.

2. **Orders were the same bug as products.** Both checkout forms fabricated
   orders into `localStorage` and never contacted the server, so stock was never
   decremented and courier consignment ids were generated by `Math.random` in
   the browser. `placeGuestOrder()` plus `POST /api/v1/orders/express` fixed the
   write path; the admin board was moved onto the API for reads, status changes,
   courier booking and call logs.

3. **Abandoned-cart leads and reviews** had the same shape and were fixed the
   same way. Two live bugs surfaced in `lib/services/incomplete-orders.ts`: a
   database failure was answered with seed data (an operator would have phoned
   fabricated leads), and `statusFilter` was accepted but never applied.

4. **Migration 0002** also caught up `drug_classifications`,
   `species_categories` and `incomplete_orders`, which existed in the Drizzle
   schema but had never been generated into a migration.

5. **`lib/jobs/` did not exist**, so `pnpm jobs` was a broken script.

### Deliberately not done

- **Customer authentication.** Phone + OTP login still writes a browser-side
  session (`lib/mock-data/auth.ts`). Orders are placed as guests, so this does
  not affect persistence. Wiring it needs the OTP/SMS path and an address book,
  which is its own piece of work.
- **Repo-wide lint debt.** 118 pre-existing `no-explicit-any` and
  `set-state-in-effect` errors remain in files this branch did not touch. The
  five components it did touch had their prop-mirroring effects removed.
